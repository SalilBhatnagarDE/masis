...
async def final_validation(state: MASISState) -> dict:
    ...
    all_pass = (scores["faithfulness"] >= 0.85 
                and scores["citation_accuracy"] >= 0.90
                and scores["answer_relevancy"] >= 0.80
                and scores["dag_completeness"] >= 0.90)
...
Task query → HyDE Rewrite → Metadata Extraction → Hybrid Retrieval (Vector + BM25 + RRF)
    → Cross-Encoder Rerank (top 5) → CRAG Document Grading (max 3 retries)
    → Generate Answer → Self-RAG Hallucination Check (max 3 retries)
...
Query → HyDE Rewrite (gpt-4.1-mini, temp 0.3)
  ...
  → CRAG Document Grading (gpt-4.1-mini, parallel, max 3 retries)
  → Generate Answer (gpt-4.1-mini, temp 0.1)
  → Self-RAG Hallucination Check (gpt-4.1-mini, max 3 retries)
  → Return structured ResearcherOutput
```
...
| Layer | Mechanism |
|---|---|
| Inside RAG (Eng 6) | CRAG max 3 retries, Self-RAG max 3 retries |
| Supervisor Fast Path | Cosine similarity > 0.9 between same-type tasks → force stop |
| Global Hard Caps | `MAX_SUPERVISOR_TURNS=15`, `max_wall_clock=300s`, budget ceiling |
...
Turn 15: Fast Path checks iteration_count=15 >= 15 → FORCE SYNTHESIZE
  Reason: "max_iterations_reached (15)"
...
async def final_validation(state: MASISState) -> dict:
    ...
    scores = {
        "faithfulness":      faithfulness,       # Threshold: 0.85
        "citation_accuracy": citation_accuracy,   # Threshold: 0.90
        "answer_relevancy":  answer_relevancy,    # Threshold: 0.80
        "dag_completeness":  dag_completeness,    # Threshold: 0.90
    }
    ...
    # MF-VAL-07: Max validation rounds (cap at 3)
...
```
Round 1:
  faithfulness: 0.72 ❌ (< 0.85) | citation_accuracy: 0.91 ✅ | relevancy: 0.88 ✅ | completeness: 0.95 ✅
...
async def run_researcher(task: TaskNode, state: dict) -> ResearcherOutput:
    ...
    # MF-RES-05: CRAG document grading (max 3 retries)
    ...
    # MF-RES-06: Self-RAG hallucination check (max 3 retries)
    for attempt in range(3):
...
LOOP_PREVENTION = {
    "layer_1": "CRAG max 3 retries inside Eng 6 (MF-RES-05)",
    "layer_2": "Supervisor cosine > 0.90 repetition detection (MF-SUP-06, Section 14.4)",
    "layer_3": "Hard cap MAX_SUPERVISOR_TURNS=15 (MF-SUP-05, Section 23.1)",
}
...
# answer_relevancy(synthesis.answer, original_query) >= 0.80
...
async def run_regression():
    ...
    if avg_faith < 0.85:
        alert(f"REGRESSION: faithfulness dropped to {avg_faith:.3f} (< 0.85)")

    return results

# MF-EVAL-03: Per-scenario testing
# Each of S1-S10 from reasoning_simulation.md as a pytest test:
# test_s1_simple_factual() → asserts faithfulness > 0.85, cost < $0.05
