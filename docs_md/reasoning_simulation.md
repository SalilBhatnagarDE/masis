...
> **Note:** Some numeric values in illustrative flows are examples for explanation; runtime thresholds/caps are defined in `masis/schemas/thresholds.py`.
...
Step 8 — VALIDATOR
  faithfulness: 0.94 ✅ (threshold: 0.00)
  citation_accuracy: 0.95 ✅ (threshold: 0.00)
  answer_relevancy: 0.97 ✅ (threshold: 0.02)
  dag_completeness: 1.00 ✅ (threshold: 0.50)
...
Step 2 — EXECUTOR → T1 ║ T2 (Parallel via Send())
  ...
  T2 (Worker 2): Researcher, HyDE → retrieval → CRAG retries 1/1 → 1 chunk relevant ❌
...
Step 2 — EXECUTOR → T1 ║ T2 ║ T3 ║ T4 ║ T5 (Parallel, 5 Send())
  ...
  T3: Patent data → CRAG 1/1 retries → pass_rate 0.05 ❌
  T4: Headcount → pass_rate 0.0, 0 relevant chunks ❌
  T5: Efficiency metrics → CRAG 1/1 retries → pass_rate 0.0 ❌
...
Step 2 — EXECUTOR → T1 (Researcher)
  ...
  ⚡ LAYER 1: CRAG max retries (1/1) exhausted inside Eng 6 pipeline
...
LAYERS THAT PREVENTED INFINITE LOOP:
  ...
  3. Hard cap: MAX_SUPERVISOR_TURNS = 10 (if layers 1+2 somehow failed)
...
Step 7 — VALIDATOR
  faithfulness: 0.71 ❌ (threshold: 0.00)
  citation_accuracy: 0.88 ❌ (threshold: 0.00)
